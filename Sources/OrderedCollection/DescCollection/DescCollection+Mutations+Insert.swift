//
//  DescCollection+Mutations+Insert.swift
//  OrderedCollection
//
...
extension DescCollection where Buffer: MutationCollectionInsert,
    Buffer: Equatable,
    Buffer.Index == Int {
    public
    ...
    mutating func insert<C>(contentsOf newElements: C, at i: Int) throws where C: Collection, Element == C.Element {
        guard let left = newElements.first, let right = try checkDescCollection(newElements) else {
            return
        }
    ...
    public
    mutating func insert(contentsOf newElements: DescCollection, at i: Int) throws {
