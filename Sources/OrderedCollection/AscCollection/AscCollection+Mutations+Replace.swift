//
//  AscCollection+Mutations+Replace.swift
//  OrderedCollection
//
//  Created by Vitali Kurlovich on 1/17/19.
//

extension AscCollection where Buffer: MutationCollectionReplace,
    Buffer: Equatable,
    Buffer.Index == Int {
    public
    ...
    public
    mutating func replaceSubrange<R>(_ subrange: R, with newElements: AscCollection) throws where R: RangeExpression, Index == R.Bound {
        guard canReplace(subrange, with: newElements) else {
            throw OrderedCollectionError.IncorrectValueError
        }
...
extension AscCollection where Buffer: MutationCollectionReplace,
    ...
    internal
    func canReplace<R>(_ subrange: R, with newElements: AscCollection) -> Bool where R: RangeExpression, Index == R.Bound {
        let range = subrange.relative(to: self)
        return canReplace(range, with: newElements)
    }
    ...
    internal
    func canReplace(_ range: Range<Index>, with newElements: AscCollection) -> Bool {
        guard let first = newElements.first else {
            return true
        }
